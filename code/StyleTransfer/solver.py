...
class Solver:
    ...
    def __init__(self,cnfg):
        torch.manual_seed(1)
        random.seed(7867567)
        styleProblems=["STYLED","ethnic","gender","education","country","tod","politics","age"]
    ...
    def main(self):
        ...
        if cnfg.mode!="inference" and cnfg.preTrain:
            preTrain_src=srcLangObj.read_corpus("pretrain")
        ...
        if cnfg.mode!="inference" and cnfg.preTrain:
            preTrain_src_batches,preTrain_src_masks=torch_utils.splitBatches(train=preTrain_src,batch_size=cnfg.batch_size,padSymbol=cnfg.garbage,method="pre")
            preTrain_tgt_batches,preTrain_tgt_masks=torch_utils.splitBatches(train=preTrain_tgt,batch_size=cnfg.batch_size,padSymbol=cnfg.garbage,method="post")
        ...
        print(len(test_tgt_batches),len(test_src_batches))
        ...
        if cnfg.mode!="inference" and cnfg.preTrain:
            preTrain_batches=list(zip(preTrain_src_batches,preTrain_tgt_batches,preTrain_src_masks,preTrain_tgt_masks))
